...
class MultiWii:
    ...
    def __init__(self, serPort):
        ...
        self.gps = {'I1':0,'I2':0,'I3':0,'I4':0,'I5':0,'I6':0,'I7':0,'I8':0,'I9':0,'I10':0,'I11':0}
        self.temp = ();
        self.temp2 = ();
        self.elapsed = 0
        self.PRINT = 1
    ...
    def getData(self, cmd):
        try:
            ...
            temp = struct.unpack('<'+'h'*(datalength/2),data)
            # print temp
            # temp_t = struct.unpack('<'+'h'*(datalength),data)
            self.ser.flushInput()
            self.ser.flushOutput()
            elapsed = time.time() - start
            ...
            if cmd == MultiWii.RAW_GPS:
            	# print "---------COMP_GPS-------\n"
            	self.gps['I1']=bin(temp[0])
                self.gps['I2']=bin(temp[1])
                self.gps['I3']=bin(temp[2])
                self.gps['I4']=bin(temp[3])
                self.gps['I5']=bin(temp[4])
                self.gps['I6']=float(temp[5])
                self.gps['I7']=float(temp[6])
